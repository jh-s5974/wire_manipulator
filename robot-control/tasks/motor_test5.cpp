...
struct Motor {
    ...
    Motor(canid_t id, uint16_t speed, float torque, float torque_const, float kp, float kd, float ready_pos, float min_pos, float max_pos)
        : can_id(id), max_speed(speed), max_torque(torque), tc(torque_const), kp(kp), kd(kd), ready_pos(ready_pos), min_pos(min_pos), max_pos(max_pos) {}
};
...
private:
    ...
    std::atomic<bool> state_flag_{false}; // 조이스틱으로 rl mode 활성화
    ...
    std::atomic<bool> stop_flag_{true}; // 조이스틱으로 stop_mode 활성화
    ...
    void initialize_motors() {
        ...
        motors_.push_back(std::make_unique<Motor>(0x151, 2,  30, 30.0f/4.0f, 240.0, 5.0,  0.0,  -0.45, 0.4));
        motors_.push_back(std::make_unique<Motor>(0x141, 2,  30, 30.0f/4.0f, 240.0, 5.0,  0.0,  -0.4, 0.45));
        motors_.push_back(std::make_unique<Motor>(0x152, 2,  30, 30.0f/4.0f, 240.0, 5.0,  0.0,  -0.15, 1.0));
        motors_.push_back(std::make_unique<Motor>(0x142, 2,  30, 30.0f/4.0f, 240.0, 5.0,  0.0,  -1.0, 0.15));
        motors_.push_back(std::make_unique<Motor>(0x153, 5,  50, 50.0f/6.7f,  50.0, 1.8, -0.2,  -1.8, 0.4));
        motors_.push_back(std::make_unique<Motor>(0x143, 5,  50, 50.0f/6.7f,  50.0, 1.8,  0.2,  -1.8, 0.4));
        motors_.push_back(std::make_unique<Motor>(0x154, 11, 50, 50.0f/6.7f,  20.0, 1.1, -0.4,  -0.01, 1.95));
        motors_.push_back(std::make_unique<Motor>(0x144, 11, 50, 50.0f/6.7f,  20.0, 1.1,  0.4,  -0.01, 1.95));
        motors_.push_back(std::make_unique<Motor>(0x156, 5,  8 , 4.0f/3.6f ,  10.0, 1.0, 0.27, -0.5, 1.0));
        motors_.push_back(std::make_unique<Motor>(0x145, 5,  8 , 4.0f/3.6f ,  10.0, 1.0, -0.27, -1.0, 0.5));
        motors_.push_back(std::make_unique<Motor>(0x155, 5,  8 , 4.0f/3.6f ,  10.0, 1.0,  0.27, -0.3, 1.0));
        motors_.push_back(std::make_unique<Motor>(0x146, 5,  8 , 4.0f/3.6f ,  10.0, 1.0,  0.27, -0.3, 1.0));
        ...
        if (msg->buttons.size() > 7){
            if (msg->buttons[4] == 1 && state_flag_.load() == false) {
                ...
                for (const auto& motor_ptr : motors_) {
                    motor_ptr->tx_scount.store(0);
                    motor_ptr->rx_scount.store(0);
                    motor_ptr->tx_pcount.store(0);
                    motor_ptr->rx_pcount.store(0);
                    motor_ptr->tx_cmd_count.store(0);
                }
    ...
    int open_can_socket(const char* interface, const std::vector<struct can_filter>& filters) {
        ...
        struct can_frame trash_frame;
        while (read(s, &trash_frame, sizeof(trash_frame)) > 0) {}
        std::cout << "CAN Buffer flushed on " << interface << std::endl;
    ...
    int motor_limit(const auto& motor_ptr){
        float current_pos = motor_ptr->pos; 
        if(current_pos < motor_ptr->min_pos || current_pos > motor_ptr->max_pos){
            stop_flag_.store(true);
            origin_flag_.store(false);
            state_flag_.store(false);
            torque_flag_.store(false);
            ready_flag_.store(false);
            RCLCPP_WARN(this->get_logger(), "Over motor position limit (id: 0x%x, pos: %f, min: %f, max: %f)",motor_ptr->can_id, motor_ptr->pos, motor_ptr->min_pos, motor_ptr->max_pos);
        }
        return 0;
    }
    ...
    void can_tx_task(int core, const char* interface_name, canid_t id_min, canid_t id_max) {
        ...
        int hz = 50; // 1000/50 = 20 -> 50hz
        // int hz = 200; // 1000/200 = 5 -> 200hz
        int hz_count = TX_FREQ_HZ / hz;
    ...
    void control_and_publish_thread(int core) {
        ...
        while(run_flag_.load()) {
            if(origin_flag_.load() == false && state_flag_.load() == false && stop_flag_.load() == false){
                for (const auto& motor_ptr : motors_) {
                    motor_limit(motor_ptr);
                }
            }
...
// sudo ./src/rlcode/src/set_active_cores.sh 1 2 3 4 5 6 7 8
