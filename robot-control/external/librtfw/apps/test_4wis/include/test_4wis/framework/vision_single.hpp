        ...
        while(run) {
            ...
            if(!ids.empty()) {
                ...
                for (int i = 0; i < nMarkers; i++) {
                    ...
                    if (solvePnP(objPoints, corners[i], cameraMatrix, distortionCoeffs, rvec, tvec, false, cv::SOLVEPNP_ITERATIVE)) {
                        ready = true;
                    } else {
            ...
            if (ready) {
                ...
                auto angle = sqrt(rvec.dot(rvec));
                Eigen::Vector3d axis;
                axis << rvec[0], rvec[1], rvec[2];
                // if (axis[2] )
                axis = axis/angle;
                Eigen::AngleAxisd aa(angle, axis);

                SE3 c_X_m(aa.matrix(), position);
                ...
                // publish_pose(pos, orient, frame_id, cal_fps);
                publish_pose(pos, orient, frame_id, fps);

                cv::aruco::drawDetectedMarkers(image, corners, ids);
                cv::aruco::drawAxis(image, cameraMatrix, distortionCoeffs, rvec, tvec, 0.1);
            } else {
                result.marker = false;
                result.stamp = std::chrono::steady_clock::now();
                cv::putText(image, "Marker Lost", cv::Point(100, 100), 0, 3, cv::Scalar(0, 0, 255), 5, 8);
            }
            signal.tx.detection.send(result.marker);

            if (on_rec) {
                // img_buf.push(std::make_shared<cv::Mat>(image));
                img_buf.Produce(std::make_shared<cv::Mat>(image));
            }
