...
private:
    void initialize() {
        int policy_core = 5;
        int policy_priority = 90;
        set_thread_affinity(policy_core, policy_priority, "policy_thread");
        RCLCPP_INFO(this->get_logger(), "Real-time priority set for policy thread (Core %d, Prio %d).", policy_core, policy_priority);
        ...
        latest_actions_scaled_.resize(cfg_.num_actions, 0.0f);
        ...
        // 50Hz 타이머 (20ms마다 action publish)
        action_timer_ = this->create_wall_timer(
            std::chrono::milliseconds(20),
            std::bind(&PolicyCalNode::publish_action, this)
        );
    ...
    void observation_callback(const msg_interfaces::msg::Observation::SharedPtr msg) {
        ...
        pre_actions_ = actions_clipped; // 다음 스텝 저장

        std::vector<float> actions_scaled(cfg_.num_actions);
        for (size_t i = 0; i < cfg_.num_actions; ++i) {
            actions_scaled[i] = actions_clipped[i] * cfg_.action_scale + cfg_.default_joint_positions[i];
        }

        latest_actions_scaled_ = actions_scaled; // <- 타이머에서 publish할 값
    }

    // 50Hz 타이머 콜백: 항상 최신 행동을 publish
    void publish_action() {
        auto action_msg = msg_interfaces::msg::Action();
        action_msg.header.stamp = this->get_clock()->now();
        action_msg.action = latest_actions_scaled_;
        action_msg.name = joint_names_;
    ...
    std::vector<float> latest_actions_scaled_;
