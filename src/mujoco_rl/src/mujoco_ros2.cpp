...
#include "msg_interfaces/msg/obs.hpp"
...
public:
    MujocoEnv() : Node("mujoco_node") {
        ...
        obs_pub_ = this->create_publisher<msg_interfaces::msg::Obs>("obs", 10);
...
private:
    ...
    void publish_observations() {
        auto obs_tensor = _get_observations();
        auto obs_msg = msg_interfaces::msg::Obs();
        const float* tensor_data = obs_tensor.data_ptr<float>();
        obs_msg.obs.assign(tensor_data, tensor_data + obs_tensor.numel());
        obs_msg.flag = close_;
    ...
    void _apply_actions(const torch::Tensor& actions) {
        ...
        if (mode_ == 1){
            ...
            target_positions = torch::zeros({cfg_.num_joints}, torch::kFloat32);
        }
        auto output_torques = joint_kp_ * (target_positions - _get_joint_pos()) + joint_kd_ * (-_get_joint_vel());

        auto output_torques_clipped = torch::clamp(output_torques, -effort_limits_, effort_limits_);
    ...
    torch::Tensor _get_observations() {
        torch::Tensor base_quat = torch::from_blob(data_->sensordata + 3 * model_->nu, {4}, torch::kDouble).to(torch::kFloat32);
        torch::Tensor base_ang_vel = torch::from_blob(data_->sensordata + 3 * model_->nu + 4, {3}, torch::kDouble).to(torch::kFloat32);
        torch::Tensor base_vel = quat_rotate_inverse(base_quat, torch::from_blob(data_->qvel, {3}, torch::kDouble).to(torch::kFloat32));
        torch::Tensor projected_gravity = quat_rotate_inverse(base_quat, gravity_vector_);
        ...
        torch::Tensor joint_pos = _get_joint_pos().index_select(0, index_tensor);
        torch::Tensor joint_vel = _get_joint_vel().index_select(0, index_tensor);

        if (cfg_.num_observations == 47){
            return torch::cat({
                clock,
                base_ang_vel,
                projected_gravity,
        ...
        else if (cfg_.num_observations == 50){
            return torch::cat({
                clock,
                base_vel,
                base_ang_vel,
    ...
    rclcpp::Publisher<msg_interfaces::msg::Obs>::SharedPtr obs_pub_;
