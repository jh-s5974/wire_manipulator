...
private:
    void initialize() {
        ...
        try {
            ...
            // std::string config_file_path = package_share_dir + "/config/robotnl_pre.yaml";
    ...
    void observation_callback(const msg_interfaces::msg::Observation::SharedPtr msg) {
        ...
        // policy_observations_.insert(policy_observations_.end(), msg->base_vel.begin(), msg->base_vel.end());
        ...
        std::cout << msg->commands << std::endl;
        ...
        if (runner_type_ == "torch") {
            ...
            at::Tensor output_tensor = policy_pt_.forward({input_tensor}).toTensor();
