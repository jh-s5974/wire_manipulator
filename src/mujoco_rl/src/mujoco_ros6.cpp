...
struct Config {
    ...
    int num_joints;
    // int num_actions; // REMOVED - No longer needed
    // std::vector<int> action_indices; // REMOVED - No longer needed
...
public:
    MujocoEnv() : Node("mujoco_node") {
        ...
        for (int i = 0; i < model_->nu; ++i) {
            if (model_->actuator_trntype[i] == mjTRN_JOINT) {
                ...
                if (joint_name) {
                    joint_name_to_idx_map_[joint_name] = i;
                    RCLCPP_INFO(this->get_logger(), "Mapped joint '%s' to actuator index %d", joint_name, i);
                }
            }
        }
        // ------------------------------------

        const char* body_name_to_track = "base_link";
        base_link_id_ = mj_name2id(model_, mjOBJ_BODY, body_name_to_track);
        if (base_link_id_ == -1) {
            RCLCPP_WARN(this->get_logger(), "Could not find body '%s' to track. Camera tracking will be disabled.", body_name_to_track);
            camera_tracking_ = false;
        } else {
            RCLCPP_INFO(this->get_logger(), "Camera will track body '%s' (ID: %d). Press 'F' to toggle.", body_name_to_track, base_link_id_);
        }
        ...
        joint_kp_ = torch::from_blob(cfg_.joint_kp.data(), {cfg_.num_joints}, torch::kDouble).to(torch::kFloat32);
        joint_kd_ = torch::from_blob(cfg_.joint_kd.data(), {cfg_.num_joints}, torch::kDouble).to(torch::kFloat32);
        effort_limits_ = torch::from_blob(cfg_.effort_limits.data(), {cfg_.num_joints}, torch::kDouble).to(torch::kFloat32);
        ...
        // --- CHANGED: 목표 조인트 위치 텐서 초기화 ---
        target_joint_positions_ = torch::from_blob(cfg_.default_joint_positions.data(), {cfg_.num_joints}, torch::kDouble).to(torch::kFloat32);
        // -----------------------------------------
    ...
    // --- CHANGED: run_step에서 action 전달 제거 ---
    void run_step() {
        step();
    }
    // -------------------------------------------
    ...
    void render() {
        ...
        if (camera_tracking_ && base_link_id_ != -1) {
    ...
    void reset() {
        ...
        for (int i = 0; i < cfg_.num_joints; ++i) {
            data_->qpos[7 + i] = cfg_.joint_reset_positions[i];
        }
...
private:
    void load_config(const std::string& path) {
        try {
            ...
            cfg_.policy_dt = yaml_config["policy_dt"].as<double>();
            cfg_.physics_dt = yaml_config["physics_dt"].as<double>();
            cfg_.num_joints = yaml_config["num_joints"].as<int>();
            // cfg_.num_actions = yaml_config["num_actions"].as<int>(); // REMOVED
            cfg_.joint_kp = yaml_config["joint_kp"].as<std::vector<double>>();
            cfg_.joint_kd = yaml_config["joint_kd"].as<std::vector<double>>();
            cfg_.default_joint_positions = yaml_config["default_joint_positions"].as<std::vector<double>>();
            cfg_.joint_reset_positions = yaml_config["joint_reset_positions"].as<std::vector<double>>();
            cfg_.effort_limits = yaml_config["effort_limits"].as<std::vector<double>>();
            // cfg_.action_indices = yaml_config["action_indices"].as<std::vector<int>>(); // REMOVED
            cfg_.num_observations = yaml_config["num_observations"].as<int>();
            cfg_.cutoff_freq = yaml_config["cutoff_freq"].as<double>();
    ...
    void joystick_callback(const msg_interfaces::msg::Joymsg::SharedPtr msg) {
        if (msg->buttons.size() > 7) {
            if (msg->buttons[0] == 1 && mode_ != 1){
                mode_ = 1;
                RCLCPP_INFO(get_logger(), "Mode changed to: [Reinforcement Learning]");
            }
            else if (msg->buttons[1] == 1 && mode_ != 0){
                mode_ = 0;
                RCLCPP_INFO(get_logger(), "Mode changed to: [Initial Position]");
            }
            tracking_state_ = msg->buttons[6];
            if(msg->buttons[6] == 1 && prev_tracking_state_ == 0){
                camera_tracking_ = !camera_tracking_;
                if (camera_tracking_) {
                    RCLCPP_INFO(get_logger(), "Camera tracking [ENABLED]");
                } else {
                    RCLCPP_INFO(get_logger(), "Camera tracking [DISABLED]");
                }
            }
    ...
    // --- CHANGED: action_callback 로직 전체 변경 ---
    void action_callback(const msg_interfaces::msg::Action::SharedPtr msg) {
        std::lock_guard<std::mutex> lock(action_mutex_);
        for (size_t i = 0; i < msg->name.size(); ++i) {
            const auto& name = msg->name[i];
            const auto& value = msg->action[i];

            // 맵에서 조인트 이름을 찾아 해당 인덱스의 목표 위치를 업데이트
            if (joint_name_to_idx_map_.count(name)) {
                int joint_idx = joint_name_to_idx_map_.at(name);
                target_joint_positions_[joint_idx] = value;
            }
        }
    }
    // -------------------------------------------

    // --- CHANGED: step 함수에서 action 인수 제거 ---
    void step() {
        // 1. 현재 스텝에서 사용할 목표 위치를 안전하게 복사
        torch::Tensor current_target_positions;
        if (mode_ == 1) {
            std::lock_guard<std::mutex> lock(action_mutex_);
            current_target_positions = target_joint_positions_.clone();
        } else {
            current_target_positions = torch::from_blob(cfg_.default_joint_positions.data(), {cfg_.num_joints}, torch::kDouble).to(torch::kFloat32);
        }
        ...
        for (int i = 0; i < substeps; ++i) {
            _apply_actions(current_target_positions); // 복사된 목표 위치 전달
    ...
    // --- CHANGED: _apply_actions 로직 전체 변경 ---
    void _apply_actions(const torch::Tensor& final_target_positions) {
        auto output_torques = joint_kp_ * (final_target_positions - _get_joint_pos()) + joint_kd_ * (-_get_joint_vel());
    ...
    // --- ADDED/CHANGED 멤버 변수 ---
    torch::Tensor target_joint_positions_; // 목표 조인트 위치 저장
    std::unordered_map<std::string, int> joint_name_to_idx_map_; // 조인트 이름-인덱스 맵
    // ---------------------------
...
void key_callback(GLFWwindow* window, int key, int scancode, int action, int mods) {
    ...
    if (key == GLFW_KEY_F && action == GLFW_PRESS) {
        ...
        if (env->camera_tracking_) {
            RCLCPP_INFO(env->get_logger(), "Camera tracking [ENABLED]");
        } else {
            RCLCPP_INFO(env->get_logger(), "Camera tracking [DISABLED]");
        }
    }
...
void mouse_move_callback(GLFWwindow* window, double xpos, double ypos) {
    ...
    mjv_moveCamera(env->model_, action_mouse, dx / height, dy / height, &env->scn_, &env->cam_);
}
