...
public:
    MujocoEnv() : Node("mujoco_node") {
        ...
        if (base_link_id_ == -1) {
            RCLCPP_WARN(this->get_logger(), "추적할 body '%s'를 찾지 못했습니다. 카메라 추적 기능이 비활성화됩니다.", body_name_to_track);
            camera_tracking_ = false;
        } else {
            RCLCPP_INFO(this->get_logger(), "카메라가 body '%s' (ID: %d)를 추적합니다. 'F' 키로 기능을 켜고 끌 수 있습니다.", body_name_to_track, base_link_id_);
        }
        ...
        auto default_positions_tensor = torch::from_blob(cfg_.default_joint_positions.data(), {cfg_.num_joints}, torch::kDouble).to(torch::kFloat32);
    ...
    void reset() {
        ...
        for (int i = 0; i < cfg_.num_joints; ++i) {
            ...
            // data_->qpos[i] = cfg_.joint_reset_positions[i];
        }
...
private:
    ...
    void joystick_callback(const msg_interfaces::msg::Joymsg::SharedPtr msg) {
        if (msg->buttons.size() > 7) {
            ...
            if(msg->buttons[6] == 1 && prev_tracking_state_ == 0){
                ...
                if (camera_tracking_) {
                    RCLCPP_INFO(get_logger(), "카메라 추적 기능 [활성화]");
                } else {
                    RCLCPP_INFO(get_logger(), "카메라 추적 기능 [비활성화]");
                }
            }
    ...
    void _apply_actions(const torch::Tensor& actions) {
        ...
        if (mode_ == 1){
            ...
            target_positions = torch::from_blob(cfg_.default_joint_positions.data(), {cfg_.num_joints}, torch::kDouble).to(torch::kFloat32);
        }
...
void key_callback(GLFWwindow* window, int key, int scancode, int action, int mods) {
    ...
    if (key == GLFW_KEY_F && action == GLFW_PRESS) {
        ...
        if (env->camera_tracking_) {
            RCLCPP_INFO(env->get_logger(), "카메라 추적 기능 [활성화]");
        } else {
            RCLCPP_INFO(env->get_logger(), "카메라 추적 기능 [비활성화]");
        }
    }
